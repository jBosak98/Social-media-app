...
public class SendMessageActivity extends BaseAunthenticatedActivity implements View.OnClickListener {
    ...
    protected void onYoraCreate(Bundle savedState) {
        ...
        recipientButton = (Button) findViewById(R.id.activity_send_message_recpient);
    ...
    public void onMessageSent(Messages.SendMessageResponse response) {
        if (!response.didSucceed()) {
            ...
            progressFrame.animate().alpha(0).setDuration(250).withEndAction(
                    new Runnable() {
                        @Override
                        public void run() {
                            progressFrame.setVisibility(View.GONE);
